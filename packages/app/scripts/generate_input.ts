...
async function getArgs() {
  ...
  const email_file = emailFileArg ? emailFileArg.split("--email_file=")[1] : "emls/test_twitter.eml";
...
async function test_generate(writeToFile: boolean = true) {
  ...
  if (writeToFile) {
    ...
    const filename = nonce ? `${file_dir}/input_${nonce}.json` : "./packages/circuits/inputs/input.json";
